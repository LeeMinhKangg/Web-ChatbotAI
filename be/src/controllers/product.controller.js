...
const getAllProducts = async (req, res, next) => {
  try {
    ...
    // Price filter - sử dụng subquery để filter theo giá trong variants
    ...
    console.log('Sort parameter:', sort, 'Order:', order);
    if (sort === 'price') {
      console.log('Sorting by price, order:', order);
      console.log('Before sort:', products.slice(0, 3).map(p => ({ name: p.name, price: p.price })));
      if (order === 'ASC') {
        products.sort((a, b) => parseFloat(a.price) - parseFloat(b.price));
      } else {
        products.sort((a, b) => parseFloat(b.price) - parseFloat(a.price));
      }
      console.log('After sort:', products.slice(0, 3).map(p => ({ name: p.name, price: p.price })));
    }
